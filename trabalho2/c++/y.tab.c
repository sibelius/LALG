...
#line 148 "y.tab.c"
  ...
  BisonParser::BisonParser (Waffleshop::FlexScanner &scanner_yyarg)
    :
#if YYDEBUG
      yydebug_ (false),
      yycdebug_ (&std::cerr),
...
#line 27 "waffleshop.y"
    { std::cout << "INTEGER / 5 = " << ((yysemantic_stack_[(2) - (2)]) / 5) << std::endl; }
    break;

  case 3:

/* Line 690 of lalr1.cc  */
#line 28 "waffleshop.y"
    { std::cout << "INTEGER / 5 = " << ((yysemantic_stack_[(1) - (1)]) / 5) << std::endl; }
    break;
...
#line 425 "y.tab.c"
  ...
  {
    "$end", "error", "$undefined", "INTEGER", "$accept", "program", 0
  };
#endif
  ...
  const int BisonParser::yylast_ = 3;
  const int BisonParser::yynnts_ = 2;
  const int BisonParser::yyempty_ = -2;
  const int BisonParser::yyfinal_ = 3;
  const int BisonParser::yyterror_ = 1;
  const int BisonParser::yyerrcode_ = 256;
  const int BisonParser::yyntokens_ = 4;
